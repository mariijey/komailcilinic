...
import { withAccessGuard } from '../../../../utils/route-guard/AccessGuard';
...
const BaseNavCollapse = ({ menu, level }: Props) => {
  ...
  const navCollapse = menu.children?.map((item) => {
    switch (item.type) {
      case 'collapse':
        return <NavCollapse access={item.access} key={item.id} menu={item} level={level + 1} />;
      case 'item':
        return <NavItem access={item.access} key={item.id} item={item} level={level + 1} />;
      default:
        return (
...
const NavCollapse = withAccessGuard(BaseNavCollapse);
